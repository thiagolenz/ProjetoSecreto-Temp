define(["components/HtmlLoader", 
        "i18n!modules/accountPatient/nls/AccountPatient.js",
...
function (htmlLoader, i18ns, i18ns_g, service, messages){
	...
	function load () {
		htmlLoader.load ({
			...
			file: "modules/accountPatient/templates/CreateAccountForm.html",
			i18n : i18ns
		}, function (){
			bindEvents();
		});
	...
	function validateAllRegisterFields () {
	    ...
	    if ($("#registerPassword").val() != $("#confirmPassword").val()){
	    	error ({
	        	value : "As senhas são diferentes"
	        });
	        return false;
	    }
	...
	function error (data) {
		$("#app-messages-container").html (data.value);
	}
