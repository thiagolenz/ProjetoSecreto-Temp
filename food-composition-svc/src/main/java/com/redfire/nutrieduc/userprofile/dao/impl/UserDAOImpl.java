...
public class UserDAOImpl extends AbstractJpaDAOImpl<User> implements UserDAO {
	private static String QUERY_FIND_DUPLICATE = "from User o where o.login = :login";
	private static final String QUERY_LOGIN = "from User o where o.login = :login and o.password = :password";
	private static final String QUERY_FIND_BY_ACCOUNT = "from User o where o.account = :account";
	private static final String QUERY_FIND_USERS = "from User o where lower(o.name) like :name or lower(o.login) like :name";
	...
	private static String QUERY_SEARCH_PATIENTS_USERS = "select u from AccountPatient ap inner join ap.patientUser u " +
			...
			" (lower(u.name) like :name or lower(u.login) like :name)";
	...
	public User findDuplicate(User user) {
		...
		parameters.put("login", user.getEmail());
	...
	public User findByUsernameAndLogin(User adminUser) {
		...
		findUserQuery.setParameter("login", adminUser.getEmail());
	...
	private void addNameWhereClause(ServiceRequest<User> request, User user,
		...
		if (!Strings.isNullOrEmpty(user.getName())) {
			strQuery.append(" and (lower(o.name) like :name or lower(o.login) like :name )");
