...
public class AccountPatientServiceImpl implements AccountPatientService {
	...
	@Transactional
	public AccountPatient getByUser(Long id) {
		User user = new User();
		user.setId(id);
		return accountPatientDAO.getByUser(user);
	}

	@Transactional
	public void update(Long id, AccountPatient accountPatient) {
		accountPatientDAO.update(accountPatient, id);
	}
